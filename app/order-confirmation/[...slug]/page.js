...
import { searchOrderAPI } from '../../lib/changeApi';
import '../order-confirmation.css'; // 作成したCSSをインポート

export default function OrderConfirmationPage() {
  const params = useParams();
  const { configuration } = useConfiguration(); // 設定情報を取得
  const [receptionNumber, year] = params.slug || [];
  ...
  useEffect(() => {
    if (receptionNumber && year) {
      setLoading(true);
      searchOrderAPI(receptionNumber, year)
        .then(data => setOrderData(data))
        .catch(err => setError(err.message))
        .finally(() => setLoading(false));
    }
  }, [receptionNumber, year]);

  const calculateOrderTotal = (order) => {
    const specialMenus = configuration?.specialMenus || {};
    const mainTotal = (order.orderItems || []).reduce((sum, item) => sum + (item.unitPrice * item.quantity), 0);
    const sideTotal = (order.sideOrders || []).reduce((sum, item) => {
        const price = specialMenus[item.productKey]?.price || 0;
  ...
  if (loading || !configuration) return <p style={{ padding: '2rem' }}>読み込み中...</p>;
  ...
  return (
    <>
      <div className="print-controls">
        <button onClick={() => window.print()} className="print-button">このページを印刷</button>
      </div>
      <div id="printable-area">
        <div className="confirmation-document compact">
          <header className="conf-doc-header">
            <div className="conf-logo">松栄寿し</div>
            <div className="conf-doc-title">
              <h1>注文確認書</h1>
              <p>受付番号: {receptionNumber || '-----'}</p>
              <p>注文の種類: {orderType || '新規注文'}</p>
            </div>
          </header>
          <section className="conf-section">
            <p className="conf-current-date">{new Date().toLocaleDateString('ja-JP')} 現在、以下の内容でご注文を受付けております。</p>
          </section>
          <section className="conf-section">
          ...
          {orders.map((order, index) => (
            <section key={order.id || index} className="conf-section conf-order-block">
              <h2 className="conf-order-title">ご注文 #{index + 1} (注文番号: {order.orderId})</h2>
              ...
              <table className="conf-items-table">
                ...
                <tbody>
                  {(order.orderItems || []).filter(item => item.quantity > 0).flatMap(item => {
                    ...
                    const rows = [];
                    if (standardQty > 0) rows.push(<tr key={`${item.productKey}-std`}><td>{item.name}</td><td>{item.unitPrice.toLocaleString()}円</td><td>{standardQty}</td><td>¥{(item.unitPrice * standardQty).toLocaleString()}</td></tr>);
                    netaChangePatterns.forEach((pattern, pIndex) => {
                      rows.push(<tr key={`${item.productKey}-${pIndex}`}><td>{item.name}{formatItemModifiers(pattern)}</td><td>{item.unitPrice.toLocaleString()}円</td><td>{pattern.quantity}</td><td>¥{(item.unitPrice * pattern.quantity).toLocaleString()}</td></tr>);
                    });
                    return rows;
                  })}
                ...
                <tfoot><tr><td colSpan="3">小計</td><td>¥{calculateOrderTotal(order).toLocaleString()}</td></tr></tfoot>
              </table>
            </section>
          ))}
