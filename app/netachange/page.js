...
const NetaChangeAdminPage = () => {
  ...
  // ★★★ 修正: ネタを追加する関数 ★★★
  const handleAddToNeta = (orderIndex, productKey, patternId, netaName) => {
    ...
    const newPatterns = targetPatterns.map(p => {
      if (p.id === patternId) {
        const currentToNeta = p.to_neta || [];
        return { ...p, to_neta: [...currentToNeta, netaName] };
      }
      return p;
    });
  ...
  // ★★★ 追加: ネタを削除する関数 ★★★
  const handleRemoveToNeta = (orderIndex, productKey, patternId, netaIndex) => {
    const newOrders = [...orders];
    const targetOrder = newOrders[orderIndex];
    const targetPatterns = targetOrder.netaChanges[productKey];
    ...
    const newPatterns = targetPatterns.map(p => {
      if (p.id === patternId) {
        const currentToNeta = [...(p.to_neta || [])];
        currentToNeta.splice(netaIndex, 1);
        return { ...p, to_neta: currentToNeta };
      }
      return p;
    });
                ...
                <div key={pattern.id} className="neta-change-card">
                  ...
                  <div className="neta-change-columns">
                    ...
                    <div className="neta-column">
                      <h4>入れるネタ</h4>
                      {/* 選択済みネタのリスト */}
                      <ul className="to-neta-list">
                        {(pattern.to_neta || []).map((neta, idx) => (
                          <li key={idx} className="to-neta-item">
                            {neta}
                            <button 
                              type="button"
                              onClick={() => handleRemoveToNeta(orderIndex, productKey, pattern.id, idx)}
                              className="remove-neta-btn"
                            >
                              ×
                            </button>
                          </li>
                      ...
                      <Select
                        ...
                        onChange={(opt) => {
                          if (opt) {
                            handleAddToNeta(orderIndex, productKey, pattern.id, opt.value);
                          }
                        }}
                      />
                    </div>
