...
const OrderForm = () => {
  ...
  // ★★★ 修正: 支払日変更時に対応する領収書の発行日も自動更新（order.idを使用）
  const updatePaymentGroup = (groupId, field, value) => {
    ...
    if (field === 'paymentDate') {
      // ★★★ 修正: valueはorder.idなので、そのまま領収書のissueDateにセット
      setManualReceipts(prev => prev.map(receipt => {
        ...
        if (receipt.groupId === groupId && !receipt.isIssueDateManuallyEdited) {
          return {
            ...receipt,
            issueDate: value  // ★★★ order.id をそのままセット
          };
        }
        return receipt;
      }));
  ...
  useEffect(() => {
    ...
    const autoGeneratedFromGroups = paymentGroupsWithTotals.map(group => {
      ...
      return {
        ...
        issueDate: group.paymentDate || '',  // ★★★ order.id がそのまま入る
    ...
    const newFinalReceipts = [...autoGeneratedFromGroups, ...purelyManualReceipts];
  ...
  // ★★★ 修正: 支払いグループがない場合の自動生成領収書（order.idを使用）★★★
  const autoGeneratedReceipts = useMemo(() => {
    const docType = getDocumentType(customerInfo.paymentMethod);
    // 支払いオプションが開いていない（通常モード）の場合
    if (!isPaymentOptionsOpen || paymentGroupsWithTotals.length === 0) {
      if (!customerInfo.paymentMethod || !customerInfo.invoiceName) return [];
      return orders.map(order => ({
        id: order.id,
        documentType: docType || '(種別未定)',
        issueDate: order.id.toString(),  // ★★★ order.id を使用（後でhandleOpenConfirmationで注文番号に変換される）
        recipientName: customerInfo.invoiceName,
        amount: calculateOrderTotal(order)
      }));
    }
    // 支払いグループがある場合は、useEffect内で処理されるのでここでは空を返す
    return [];
  }, [orders, customerInfo.paymentMethod, customerInfo.invoiceName, isPaymentOptionsOpen, paymentGroupsWithTotals.length, calculateOrderTotal]);
  ...
  const handleOpenConfirmation = async () => {
    ...
    try {
      ...
      // ★★★ 修正: 領収書のissueDateをorder.idから注文番号に変換 ★★★
      ...
      // ★★★ 支払いグループのpaymentDateも同様に変換 ★★★
      const transformedPaymentGroups = paymentGroupsWithTotals.map(group => {
        if (!group.paymentDate) {
          return group;
        }
        const correspondingOrderIndex = orders.findIndex(o => o.id.toString() === group.paymentDate.toString());
        const correspondingOrder = orders[correspondingOrderIndex];
        if (correspondingOrder) {
          const finalOrderNumber = generateOrderNumber(correspondingOrder, newReceptionNumber, correspondingOrderIndex);
          if (finalOrderNumber !== '---') {
