...
const handleOpenConfirmation = async () => {
    if(!customerInfo.floorNumber){
      customerInfo.floorNumber = '0'; // 文字列の方が安全
    }
    if (!allocationNumber) {
      alert('住所を選択してください。');
      return;
    ...
    try {
      ...
      const transformedReceipts = (manualReceipts.length > 0 ? manualReceipts : autoGeneratedReceipts).map(receipt => {
        const correspondingOrderIndex = orders.findIndex(o => 
          (receipt.groupId && o.id === parseInt(receipt.issueDate, 10)) || // グループからの場合
          (!receipt.groupId && o.orderDate === receipt.issueDate) // 自動生成の場合
        );
