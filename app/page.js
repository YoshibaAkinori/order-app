...
const OrderForm = () => {
  ...
  const autoGeneratedReceipts = useMemo(() => {
    if (!customerInfo.paymentMethod || !customerInfo.invoiceName) {
      return [];
    }
    const docType = getDocumentType(customerInfo.paymentMethod);
    if (!docType) return [];

    if (isCombinedPaymentSummaryOpen && paymentGroupsWithTotals.length > 0) {
  ...
  }, [orders, paymentGroupsWithTotals, customerInfo.paymentMethod, customerInfo.invoiceName, isCombinedPaymentSummaryOpen]);
